...
(function() {
  var CompositeToken, LoopToken, MoveToken, RandomMoveToken, ResetPositionToken, ResetToken, Segment, SetLengthToken, StateManager, Token, Vector, drawLine, fragShaderSource, getInnerSegments, getSegments, getSimpleToken, getToken, get_shader, gl, initBuffer, initBuffers, initGL, initScene, initShaders, peel, peelable, shaderProgram, stateManager, unbindBuffers, vtxShaderSource,
  ...
  StateManager = (function() {
    ...
    StateManager.prototype.resetPosition = function() {
      return this.position = Vector.origin;
    };
  ...
  Token = (function() {
    ...
    Token.prototype.setString = function(stringOverride) {
      this.stringOverride = stringOverride;
      return this;
    };

    Token.prototype.toString = function() {
      return this.stringOverride || "token";
    };

    Token.set = function(str, token) {
      return this.tokenMap[str] = token;
    };
  ...
  LoopToken = (function(_super) {
    __extends(LoopToken, _super);
    ...
    LoopToken.prototype.toString = function() {
      return "" + this.val + ":" + (this.childToken.toString());
    };
  ...
  CompositeToken = (function(_super) {
    __extends(CompositeToken, _super);
    ...
    CompositeToken.prototype.toString = function() {
      return "(" + (_.map(this.childTokens, function(token) {
  ...
  RandomMoveToken = (function(_super) {
    ...
    RandomMoveToken.prototype.toString = function() {
      return 'rand';
    };
  ...
  SetLengthToken = (function(_super) {
    ...
    SetLengthToken.prototype.toString = function() {
  ...
  ResetPositionToken = (function(_super) {
    __extends(ResetPositionToken, _super);

    function ResetPositionToken() {
      return ResetPositionToken.__super__.constructor.apply(this, arguments);
    }

    ResetPositionToken.prototype["do"] = function() {
      return stateManager.resetPosition();
    };

    ResetPositionToken.prototype.toString = function() {
      return "resetPosition";
    };

    return ResetPositionToken;
  ...
  ResetToken = (function(_super) {
    ...
    ResetToken.prototype.toString = function() {
      return "reset";
    };
  ...
  getSegments = function(codeString) {
    var currentSegment, numParens, segments;
    numParens = 0;
    currentSegment = "";
    segments = [];
    _.forEach(codeString, function(character) {
      if (character === ";" && numParens === 0) {
        ...
        return currentSegment += character;
      }
    });
    if (currentSegment.length > 0) {
      segments.push(currentSegment);
    }
    return _.map(segments, function(x) {
      return x.trim();
    });
  };

  peelable = function(codeString) {
    var indexOfFirstParen, indexOfLastParen;
    codeString = codeString.trim();
    indexOfFirstParen = _.indexOf(codeString, "(");
    indexOfLastParen = _.lastIndexOf(codeString, ")");
    return indexOfFirstParen === 0 && indexOfLastParen === codeString.length - 1;
  };

  peel = function(codeString) {
    return codeString.slice(1, codeString.length - 1);
  };

  getInnerSegments = function(codeString) {
    var innerCodeString;
    codeString = codeString.trim();
    return innerCodeString = codeString.slice(indexOfFirstParen + 1, indexOfLastParen);
  };

  getSimpleToken = function(singleSegmentString) {};

  getToken = function(segmentString) {
    var binding, innerString, loopVal, rest, segment, segmentParts, segments, val;
    if (peelable(segmentString)) {
      innerString = peel(segmentString);
      segments = getSegments(innerString);
      if (segments.length === 1) {
        return getToken(_.first(segments));
      } else {
        return new CompositeToken(_.map(segments, function(segment) {
          return getToken(segment);
        }));
      }
    } else {
  ...
  $(document).ready(function() {
    ...
    return $('#prompt').keypress(function(event) {
      var assignmentParts, codeText, codeToken, promptText, segmentStrings, varName;
      if (event.keyCode === 13) {
        promptText = $('#prompt').val();
        assignmentParts = promptText.split("=");
        if (assignmentParts[1] != null) {
          varName = assignmentParts[0].trim();
          codeText = assignmentParts[1].trim();
          segmentStrings = getSegments(codeText);
          codeToken = new CompositeToken(_.map(segmentStrings, function(segment) {
            return getToken(segment);
          }));
          ...
          initScene();
          stateManager.reset();
          _.forEach(segmentStrings, function(segmentString) {
            return getToken(segmentString)["do"]();
          });
        }
        $('#sidebar').html('');
        return _.forEach(Token.tokenMap, function(val, key) {
          var displayString;
          displayString = "" + key + ": " + (val.toString());
          return $('#sidebar').append("<div>" + displayString + "</div>");
        });
      }
    });
  });
